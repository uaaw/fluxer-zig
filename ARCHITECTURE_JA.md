...
- TLS暗号化は `gateway/shard.zig` で `std.crypto.tls.Client` を使用して実装されています。TLSハンドシェイクはOSのCAバンドル（`std.crypto.Certificate.Bundle.rescan`）を使用し、すべてのWebSocketフレームの送受信はTLSレイヤーを経由します
