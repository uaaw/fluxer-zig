...
pub const EventDispatcher = struct {
    ...
    pub fn dispatch(self: *EventDispatcher, payload: GatewayPayload) void {
        ...
        if (std.mem.eql(u8, t, "READY")) {
            const parsed = std.json.parseFromValue(ReadyPayload, self.allocator, data, .{}) catch return;
            defer parsed.deinit();
            if (self.cache) |cache| {
            ...
            const parsed = std.json.parseFromValue(Message, self.allocator, data, .{}) catch return;
            defer parsed.deinit();
            if (self.cache) |cache| {
            ...
            const parsed = std.json.parseFromValue(MessageDeletePayload, self.allocator, data, .{}) catch return;
            defer parsed.deinit();
            if (self.cache) |cache| {
            ...
            const parsed = std.json.parseFromValue(Guild, self.allocator, data, .{}) catch return;
            defer parsed.deinit();
            if (self.cache) |cache| {
            ...
            const parsed = std.json.parseFromValue(GuildDeletePayload, self.allocator, data, .{}) catch return;
            defer parsed.deinit();
            if (self.cache) |cache| {
            ...
            const parsed = std.json.parseFromValue(ChannelDeletePayload, self.allocator, data, .{}) catch return;
            defer parsed.deinit();
            if (self.cache) |cache| {
            ...
            const parsed = std.json.parseFromValue(GuildMember, self.allocator, data, .{}) catch return;
            defer parsed.deinit();
            if (self.cache) |cache| {
                const guild_id = if (data.object.get("guild_id")) |gv| blk: {
                    const parsed_guild_id = std.json.parseFromValue(Snowflake, self.allocator, gv, .{}) catch break :blk null;
            ...
            const parsed = std.json.parseFromValue(GuildMemberRemovePayload, self.allocator, data, .{}) catch return;
            defer parsed.deinit();
            if (self.cache) |cache| {
...
test "EventDispatcher dispatches READY" {
    ...
    ;
    const parsed = try std.json.parseFromSlice(GatewayPayload, std.testing.allocator, json, .{});
    defer parsed.deinit();
